 ...
 public function getAll($w=null)
  {
    ...
    try
    {
    $sql = "SELECT id,nombre,apellido,DATE_FORMAT(fecha_nac, '%d/%m/%Y %H:%i') as fecha_nac,peso,telefono1,dni,sexo,direccion,ciudad,fch_alta,id_usu_alta,estado,usuarioid FROM clientes".$where;
    $gsent = $this->db->prepare($sql);
    $gsent->execute();
 ...
 public function getOne($id)
  {
    try
    {
    $sql = "SELECT id,nombre,apellido,DATE_FORMAT(fecha_nac, '%d/%m/%Y') as fecha_nac,peso,telefono1,dni,sexo,direccion,ciudad,fch_alta,id_usu_alta,estado,usuarioid FROM clientes where id=?";
    $gsent = $this->db->prepare($sql);
